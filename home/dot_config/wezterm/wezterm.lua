...
local function setup_visuals(config)
    ...
    -- Cell backgrounds painted by TUIs (nvim, grok, ...); default 1.0 is opaque
    config.text_background_opacity = default_opacity
    config.colors = {
    ...
    config.keys = {
        ...
        { key = 'R', mods = 'CTRL', action = act.ShowDebugOverlay },
        {
            ...
            action = wezterm.action_callback(function(window, pane)
                ...
                local opacity = overrides.window_background_opacity == 1.0
                    and default_opacity
                    or 1.0
